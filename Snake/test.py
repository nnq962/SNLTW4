    ...
    def on_loop(self):
        ...
        for i in range(0, self.snake.length):
            if is_collision(self.apple.x, self.apple.y, self.snake.x[i], self.snake.y[i]):
                ...
                self.snake.up_score()
                self.snake.length += 1
                pygame.mixer.music.load("sfx_hit.wav")
        ...
        for i in range(2, self.snake.length):
            if is_collision(self.snake.x[0], self.snake.y[0], self.snake.x[i], self.snake.y[i]):
                ...
                pygame.mixer.music.load("sfx_die.wav")
                pygame.mixer.music.play(1)

    def score_display(self):
        if self.game_active:
            font = pygame.font.Font("font.TTF", 36)  # Tạo đối tượng font
            ...
            self.display_surface.blit(text, (80, 10))  # Vẽ văn bản tại vị trí (10, 10)
