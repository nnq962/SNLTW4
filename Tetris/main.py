...
class Tetris:
    ...
    # Creates a new block
    def new_block(self):
        self.block = Block(3, 0, 1)

    def next_block(self):
        self.nextBlock = Block(3, 0, 1)

    def intersects(self):
        """
        Checks if the blocks touch the top of the board
        :return: true if the blocks intersect
        """
        intersection = False
        for i in range(4):
            for j in range(4):
    ...
    # Checks if a row is formed and destroyed that line
    ...
    def draw_next_block(self, screen):
        ...
        _ = font.render("Next Shape", 1, (128, 128, 128))

        sx = topLeft_x + gameWidth + 50
        sy = topLeft_y + gameHeight / 2 - 100
        _ = self.nextBlock.image()
        for i in range(4):
            for j in range(4):
                p = i * 4 + j
    ...
    def freeze(self):
        ...
        # If blocks touch the top of the board, then ending the game by setting status as game over
        if self.intersects():
...
pygame.display.set_caption("Tetris by QDZ")
run = True
while run:
